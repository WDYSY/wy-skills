...
2. First `.env` found in this order:
   ...
   - Skill directory (e.g. `~/.agents/skills/pptx-generator/.env`)
   - Clone repo root
   - `~/.pptx-generator/.env`
...
`image_search.py` uses the same `.env` lookup order as `image_gen.py`, so skill installs can keep `PEXELS_API_KEY` / `PIXABAY_API_KEY` in `~/.pptx-generator/.env`.
