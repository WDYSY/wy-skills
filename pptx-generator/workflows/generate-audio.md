...
- High-quality cloud mode: provider API key is set before use:
  ...
  - Keys may live in the current process environment or the first `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/skills/pptx-generator/.env`), clone repo root, `~/.pptx-generator/.env`
...
```bash
python3 skills/pptx-generator/scripts/notes_to_audio.py --list-voices --locale <locale>
```

**ElevenLabs backend**:

```bash
python3 skills/pptx-generator/scripts/notes_to_audio.py --provider elevenlabs --list-voices
```

**Cloud providers using explicit voice IDs/names**:

```bash
python3 skills/pptx-generator/scripts/notes_to_audio.py --provider minimax --list-voices
python3 skills/pptx-generator/scripts/notes_to_audio.py --provider qwen --list-voices
python3 skills/pptx-generator/scripts/notes_to_audio.py --provider cosyvoice --list-voices
```
...
python3 skills/pptx-generator/scripts/notes_to_audio.py <project_path> \
...
python3 skills/pptx-generator/scripts/svg_to_pptx.py <project_path> \
...
- (If skipped embedding) one-line hint on how to embed later: `python3 skills/pptx-generator/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.
