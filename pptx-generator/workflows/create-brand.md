...
> Standalone preset-creation workflow. Output is a brand template package at `skills/pptx-generator/templates/brands/<brand_id>/`. Structurally a brand is a template minus the SVG page roster — Strategist locks the brand's color / typography / logo / voice as truth; Executor designs pages freely under those constraints.
...
| Brand site URL | `python3 skills/pptx-generator/scripts/source_to_md/web_to_md.py <URL>`, then `Read` the result | color references, font references, voice/tone |
| Branded PPTX | `python3 skills/pptx-generator/scripts/source_to_md/ppt_to_md.py <file>`, then read theme XML | colors, typography (literal) |
| Brand PDF | `python3 skills/pptx-generator/scripts/source_to_md/pdf_to_md.py <file>` | voice/tone; sometimes color/font references |
...
```bash
mkdir -p "skills/pptx-generator/templates/brands/<brand_id>"
```
...
```markdown
## ✅ Brand Saved
- Path: `skills/pptx-generator/templates/brands/<brand_id>/`
- Files: design_spec.md{, logo.<ext>}{, images/}{, illustrations/}{, icons/}
- Fields locked: <list>
- Provenance: <fact / approx / user counts>

How to use in a project:
- Include the brand directory path in your initial Step 3 input — e.g. "做一个 Q4 总结 PPT, 用 skills/pptx-generator/templates/brands/<brand_id>/ 这个品牌"
...
- To edit: modify `templates/brands/<brand_id>/design_spec.md` directly, then re-run `python3 skills/pptx-generator/scripts/register_template.py --kind brand <brand_id>`
```
