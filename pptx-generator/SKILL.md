...
description: >
  ...
  "生成PPT", "做PPT", "制作演示文稿", or mentions "pptx-generator".
---
...
> ## 🚫 严格禁止的快捷行为（中文补充强制约束）
>
> **以下行为被明确禁止，违反任何一条即构成执行失败：**
>
> 11. **禁止跳步或合并步骤** — 严格按照 Step 1 → 2 → 3 → 4 → 5 → 6 → 7 的顺序执行，任何步骤都不得跳过、合并或省略。即使用户表达"快点"、"直接生成"、"跳过确认"，也 MUST 完成每个步骤的完整输出后才能进入下一步。
>
> 12. **禁止用脚本批量生成 SVG 替代手写** — Step 6 Executor 阶段，每一页 SVG 都必须由 AI 主体逐页手写生成，含完整内容、排版和设计元素。用 Python/Node/Shell 脚本循环生成占位 SVG、空白 SVG、仅含文件名的 SVG，均视为严重违规。
>
> 13. **禁止生成空白或无内容的 PPT** — 最终导出的 PPTX 中每一页都必须包含从源文档中提取的真实内容（文字、数据、结构）。生成空白页、仅含标题、或内容与源文档无关的 PPT 均视为执行失败。
>
> 14. **禁止在 Step 6 中省略任何幻灯片页面** — spec_lock.md 中定义了多少页，就必须生成多少页完整 SVG，不得以"篇幅限制"、"节省 token"、"生成框架"为由减少页数或用占位内容代替。
>
> 15. **Step 6 每页 SVG 必须包含源文档的真实内容** — 每页 SVG 中的文字必须来自 sources/ 目录下的源文档（不得是拼音、英文占位符、"[content here]" 或随意编造的内容）。生成每页前必须重新读取 spec_lock.md 并参照 content.md 中对应章节的内容。
>
> 16. **禁止在中途停止或要求用户"手动补充"** — Step 5 的图片获取、Step 6 的 SVG 生成、Step 7 的导出，必须连续自动完成。不得中途停下来告诉用户"请手动下载图片"、"请自行运行脚本"后就不继续了；如有图片缺失，应先用纯色/渐变背景占位并继续生成，最终告知用户可替换的文件路径。

> [!IMPORTANT]
> ## 🖼 Mandatory Cover Background (Rule 17)
>
> **17. EVERY GENERATED DECK'S COVER CARRIES AN AI HERO BACKGROUND** — When a Generate run (Default or explicit Quick) has a cover in its page roster, page 1 MUST reference one AI-generated cover background produced as `page_role: hero_page` + §4.1 Primitive D (atmospheric backdrop) with `text_policy: none`. The cover's title / subtitle / numbers stay SVG text and are never baked into the image. Engine preference: the Qwen backend — `qwen-image-2.0-pro`, the model the user calls "image2.0" — through Path A, at `image_size: 2K` and the deck canvas ratio. When `IMAGE_BACKEND` (or `QWEN_API_KEY` / `DASHSCOPE_API_KEY`) is not configured, do NOT stop and do NOT ask: produce the cover background through the resolved fallback path (Path B host-native, then Offline Manual) and name the engine that actually produced the image in the final delivery. Keep the cover row engine-agnostic — same prompt, same filename, same contract — so a later Qwen run regenerates it without touching any SVG. Two documented exceptions: the user explicitly confirmed `image_usage: none` (or asked for no images), and the routes that must preserve an existing design — Beautify (1:1), template-fill (native source design), and Image to PPTX (page restoration).
...
>
> - `pptx-generator` is a repository-specific workflow, not a general application scaffold
...
**Chat-fallback content rule (Mandatory — no lossy compression to a meta-menu)**: each chat-fallback tier MUST restate every field's actual recommended value inline as plain visible text — the same values you wrote to `recommendations.json` (canvas / mode / visual_style / palette HEX / typography / page count / icons / image plan, per the current tier). Collapsing the tier into a handful of generic meta-choices ("按推荐方案直接生成" / "换个风格" / "换个配色" / "先审核") **without** restating the underlying values is a violation even if the host's reply widget is single-select — the itemized values belong in the message body regardless of what the reply widget looks like; the widget's options are for capturing the user's *decision*, never a substitute for showing the *content* being decided on.

**Chat-fallback delivery rule (Mandatory)**: some hosts render only the turn's terminal text as the visible reply and fold text that is followed by a *Confirm UI* tool call (a browser launch / `--wait` / `--wait-only`) into a collapsed "thinking" trace the user never sees. The restriction is narrow and specific: after writing a chat-fallback tier's recommendation + confirmation question, do **not** run `confirm_ui/server.py` in any form — that specific call is what strands the message behind a spinner nobody can reach. If a host is already known to have no way to hand the running user a clickable local URL (embedded / hosted enterprise agent surfaces without a desktop browser channel to that user, not just "no GUI on this machine"), skip the Confirm UI launch attempt entirely and go straight to chat fallback — do not wait out the launch/wait timeout first.

This rule does **not** forbid using the host's own native reply capability — a quick-reply card, choice buttons, a "选项卡回复"-style widget, or anything similar the current host offers — to capture the user's decision on that same recommendation. Invoke it freely, and prefer it over plain text whenever the host has one: a tappable option beats making the user type "按 A". That call is not a Confirm UI relaunch, carries no wait on an unreachable browser, and does not reintroduce the collapsing problem. Just make sure its options still satisfy the content rule above — real per-field values or a genuine per-axis pick, never a fresh coarse meta-menu invented to fit the widget.
