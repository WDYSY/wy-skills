...
> 4. Search for each concept's filename in the chosen library: `ls skills/pptx-generator/templates/icons/<chosen-library>/ | grep <keyword>`
> 5. Use the verified filename (without `.svg`) as the icon name; always include the library prefix (e.g., `chunk-filled/home`).
> 6. **Copy each chosen icon into the project as you confirm it** — `python3 skills/pptx-generator/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
...
python3 skills/pptx-generator/scripts/latex_render.py <project_path>
```
...
🚧 **GATE — before writing §VIII Image Resource List**: when image approach is B/C/D/E (anything other than A "no images"), this is a four-layer hard requirement, not a suggestion:
...
4. **Cover row is mandatory** — every deck whose §IX roster has a cover page MUST carry exactly one cover-background row: `filename: cover_bg.png`, `Acquire Via: ai`, the cover as its page, an atmosphere / image-as-canvas `Layout pattern`, and a `Reference` written as subject + intent + composition with an explicit "calm area reserved for the SVG title overlay" cue. The cover's title / subtitle / numbers are SVG text, never in-image copy. Omit this row only when the confirmed `image_usage` excludes `ai` (or the user asked for no images). Contract and engine rules: SKILL.md rule 17 and [image-generator.md](./image-generator.md) §7.
